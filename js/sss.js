/* Change the logo content according to the screen size */
function updateLogoContent() {
    const logoElement = document.getElementById("nav-logo");
    const screenWidth = window.innerWidth;

    if (screenWidth >= 768) {
      logoElement.innerText = "Felipe Buriticá";
    } else {
      logoElement.innerText = "Felipe";
    }
  }

  // Actualizar el contenido del logo cuando se cargue la página y cuando cambie el tamaño de la pantalla
  window.addEventListener("load", updateLogoContent);
...
function handleMenuClick(event) {
    ...
    const offset = 75;
    const targetPosition = targetSectionPosition - offset;
